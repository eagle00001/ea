...
public class MemcacheProxyImpl extends DefaultCacheProxyAdapter {
	...
	public void putInCache(String key, Object content, int expairTime) {
		paramNullCheck(key,"The memcache key is not allowed null.");
		paramNullCheck(content,"The memCache value is not allowed null.");
		
		try {
			if(this.listener!=null){
				if(this.listener.beforePutInCache(key, content, expairTime)){
					this.client.setWithNoReply(key, expairTime, content);
				}
			}else{
				this.client.setWithNoReply(key, expairTime, content);
			}
	 ...
	 */
	@Override
	public boolean putInCachWithReply(String key, Object val, int expairTime) {
		paramNullCheck(key,"The memcache key is not allowed null.");
		paramNullCheck(val,"The memCache value is not allowed null.");
		
		try {
			if(this.listener!=null){
				if(this.listener.beforePutInCache(key, val, expairTime)){
					return this.client.set(key, expairTime, val);
				}
			}else{
				return this.client.set(key, expairTime, val);
			}
	...
	public <T> Map<String, T> multiGet(Collection<String> keys) {
		try {
			...
			log.error("Memcached multiGet curred errors:", e);
			return null;
		}
	}
	...
	public void removeEntry(String key) {
		try {
			this.client.deleteWithNoReply(key);
			if(this.listener!=null){
				this.listener.afterRemoveEntry(key);
			}
		} catch (Exception e) {
