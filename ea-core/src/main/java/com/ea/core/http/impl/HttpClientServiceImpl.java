...
import com.ea.core.http.HttpClientService;
...
public class HttpClientServiceImpl implements HttpClientService,InitializingBean,DisposableBean {
	...
	private String httpCfgPath;
	
	public String getHttpCfgPath() {
		return httpCfgPath;
	}

	public void setHttpCfgPath(String httpCfgPath) {
		this.httpCfgPath = httpCfgPath;
	}
	...
	@Override
	public <T> T httpClientPost(String httpUrl, Map<String, String> paramsValPair,
			Class<T> t) {
		return httpClientPost(httpUrl,paramsValPair,null,t);
	}

	@Override
	public <T> T httpClientPost(String httpUrl, Map<String, String> paramsValPair,
			Map<String, String> headerMap, Class<T> t) {
		String responseStr = this.httpClientPost(httpUrl, paramsValPair, headerMap);
		if(StringUtils.isBlank(responseStr)){
			return null;
		}
		return JSONObject.parseObject(responseStr, t);
	}

	@Override
	public <T> T httpClientGet(String httpUrl, Class<T> t) {
		String responseStr = this.httpClientGet(httpUrl);
		if(StringUtils.isBlank(responseStr))
			return null;
	...
	private HttpClient initHttpConfig(){
		...
		if(StringUtils.isBlank(this.httpCfgPath)){
			logger.info("load default httpClient.properties as httpParams.");
			this.httpCfgPath = "httpClient.properties";
		}
		
		Map<Object, Object> params = FileLoadUtils.loadFileWithMap(httpCfgPath);
